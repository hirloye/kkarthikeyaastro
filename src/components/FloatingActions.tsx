...
export default function FloatingActions() {
  ...
  // Hide on admin portal and chat page
  ...
  return (
    <>
      ...
      <div className="hidden sm:block pointer-events-none fixed inset-0 z-50">
        {/* Chat Button - Left */}
        <motion.div 
          ...
          >
            ...
            >
              <span className="absolute inset-0 rounded-full bg-indigo-500 animate-ping opacity-20 group-hover:opacity-40" />
      ...
      <div className="sm:hidden fixed bottom-6 left-0 right-0 z-50 px-4 pointer-events-none">
        <div className="flex justify-center gap-3 max-w-sm mx-auto pointer-events-auto">
          <Link 
            href="/chat"
            className="flex-1 flex items-center justify-center gap-2 py-3.5 px-2 rounded-xl bg-indigo-600 text-white shadow-[0_4px_15px_rgba(99,102,241,0.4)] border border-indigo-400/30 active:scale-95 transition-all text-[11px] font-bold uppercase tracking-wider"
          >
            <MessageSquare className="w-4 h-4" />
            <span className="truncate">Chat with Astrologer</span>
          </Link>
          <a 
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 flex items-center justify-center gap-2 py-3.5 px-2 rounded-xl bg-emerald-500 text-white shadow-[0_4px_15px_rgba(16,185,129,0.4)] border border-emerald-400/30 active:scale-95 transition-all text-[11px] font-bold uppercase tracking-wider"
          >
            <Phone className="w-4 h-4" />
            <span className="truncate">Call with Astrologer</span>
          </a>
        </div>
      </div>
    </>
